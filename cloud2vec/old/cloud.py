...
def plot_words(model,basis,colors=None,old_color='black',new_color='red',target_word='target_word',target_color='green'):
	maxw = 50
	tmp_dict = {}
	word_map = [ model.wv.similar_by_vector(v) for v in basis ][0:maxw]
	full_dict = multidict.MultiDict()
	for i,v in enumerate(word_map):
		for j,w in enumerate(v):
			val = tmp_dict.get(w[0], 0)
			tmp_dict[w[0]] = val + w[1]
	for key in tmp_dict:
		tmp_dict[key] = int(tmp_dict[key] * (10**5))
	for key in tmp_dict:
